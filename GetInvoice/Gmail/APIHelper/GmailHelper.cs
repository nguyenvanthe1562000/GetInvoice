...
namespace GmailAPI.APIHelper
{
    public static class GmailAPIHelper
    {
        ...
        public static GmailService GetService()
        {
            ...
            string appDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Convert.ToString(ConfigurationManager.AppSettings["ClientInfo"])) ;
           
            using (FileStream stream = new FileStream(Path.Combine(Environment.CurrentDirectory, Convert.ToString(ConfigurationManager.AppSettings["ClientInfo"])), 
                FileMode.Open, FileAccess.Read))
            {
                String FolderPath =Path.Combine(Environment.CurrentDirectory, Convert.ToString(ConfigurationManager.AppSettings["CredentialsInfo"])); 
        ...
        public static List<string> GetAttachments(string userId, string messageId, String outputDirXML, String outputDirPdf)
        {
            try
            {
                ...
                foreach (MessagePart part in parts)
                {
                    if (!String.IsNullOrEmpty(part.Filename))
                    {
                        ...
                        part.Filename = ReplaceInvalidFileNameChars(part.Filename);
                        if(part.Filename.EndsWith(".xml"))
                        {
                            File.WriteAllBytes(Path.Combine(outputDirXML, part.Filename), data);
                        }   
                        else
                        {
                            File.WriteAllBytes(Path.Combine(outputDirPdf, part.Filename), data);
                        }
